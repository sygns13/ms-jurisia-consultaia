...
/**
 * Objeto de transporte (DTO) publicado en Kafka (tópico judicial-metrics-califications) cuando se
 * califica una demanda. Contiene todos los campos de {@link DemandasCalificadas} (excepto la
 * relación Configurations, que se aplana a {@code configurationsId}) más los datos del usuario que
 * generó la calificación, obtenidos de la sesión ({@link UserLogin}).
 */
@Schema(description = "Demanda calificada enriquecida con datos del usuario, para publicación en Kafka")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class DemandasCalificadasToKafka {

    // ===== Campos de DemandasCalificadas =====
    private Long id;
    private Long nUnico;
    private Long userId;
    ...
    /**
     * Construye el objeto a publicar a partir de la demanda calificada y el usuario de la sesión.
     * La relación Configurations se aplana a {@code configurationsId}.
     */
    public static DemandasCalificadasToKafka from(DemandasCalificadas d, UserLogin u) {
        DemandasCalificadasToKafka k = new DemandasCalificadasToKafka();
